...
// contains "reserves" of water and oxygen users can take on board
// for a one day survival expedition as well as the toggle for a 
// magnetic field that can be either installed or got rid of
    ...
    return (
        <div className="reserves-container">
            ...
            <ReserveComponent key={index} header={key} counter={data.counter} 
                                onIncrease={() => reservesDispatch({type: `INCREASE_${key.toUpperCase()}`})}
                                onDecrease={() => reservesDispatch({type: `DECREASE_${key.toUpperCase()}`})}/>)}            
        </div>
    )
}